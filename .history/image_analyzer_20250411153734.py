...
def process_gallery(gallery_name):
    ...
    gallery_path = project_root / "public" / "images" / gallery_name
    ...
    image_files = list(gallery_path.glob("*.jpg")) + \
                 list(gallery_path.glob("*.JPG")) + \
                 list(gallery_path.glob("*.webp")) + \
                 list(gallery_path.glob("*.WEBP"))
    ...
    output_file = project_root / f"{gallery_name}_descriptions.json"
