...
# Using Salesforce's BLIP model (smaller version)
...
class GalleryAnalyzer:
    ...
    def enhance_description(self, base_description: str, category: str) -> str:
        ...
        openings = [
            f"Defying convention, this {adj1} {category} unleashes {adj2} energy through",
            f"A revelation in form, this {category} transcends ordinary design with its {adj1}",
            f"Breaking boundaries between {adj1} and {adj2}, this {category} manifests",
            f"Sculpted from darkness, this {category} embodies {adj1} precision with",
            f"In a dance of {adj1} elements, this {category} weaves {adj2} allure through",
            f"Merging shadow and structure, this {category} channels {adj1} power via",
            f"Like liquid darkness made solid, this {category} captures {adj1} motion in",
            f"A symphony of {adj1} angles, this {category} commands attention with its",
            f"Born from void and vision, this {category} radiates {adj1} presence through",
            f"Challenging reality's edges, this {category} births {adj1} chaos in"
        ]
        
        # Different material description templates
        material_desc = random.choice([
            f"Crafted from {material1} fused with {material2}",
            f"A marriage of {material1} and {material2}",
            f"Forged where {material1} meets {material2}",
            f"{material1} intersecting with {material2}",
            f"Layers of {material1} dancing with {material2}"
        ])
        
        # Different element description templates
        element_desc = random.choice([
            f"featuring {element1} that flow into {element2}",
            f"where {element1} intertwine with {element2}",
            f"dominated by {element1} accented with {element2}",
            f"marked by {element1} flowing into {element2}",
            f"defined by {element1} embracing {element2}"
        ])
        
        # Different closing templates
        closings = [
            f"Reality observed: {base_description}",
            f"In essence: {base_description}",
            f"At its core: {base_description}",
            f"Physical form: {base_description}",
            f"Manifest as: {base_description}"
        ]
    ...
    def generate_image_description(self, image_path: str, category: str) -> str:
        ...
        try:
            ...
            # Request a detailed description with category-specific prompt
